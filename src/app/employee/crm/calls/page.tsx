...
import { Phone, Calendar, Clock, Search, ArrowRight, AlertTriangle, ShieldCheck, Headphones } from 'lucide-react';
import Link from 'next/link';

export default function EmployeeCallSessionsPage() {
  ...
  const loadCalls = useCallback(async () => {
    ...
    try {
      ...
      console.error('Error loading call sessions:', e.message || e);
    } finally {
      setLoading(false);
    }
  }, [employee]);
  ...
  const filtered = calls.filter(c => {
    ...
    const matchesSearch = customerName.includes(q) || car.includes(q) || review.includes(q);
    ...
    <div style={{ width: '100%', maxWidth: '1400px', margin: '0 auto', fontFamily: "'Outfit', sans-serif", padding: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--db-tx, #000)', margin: 0, display: 'flex', alignItems: 'center', gap: '10px' }}>
            <Headphones size={28} style={{ color: '#E10613' }} /> My Call Sessions
          </h1>
          <p style={{ fontSize: '0.875rem', color: 'var(--db-tx2, #555)', marginTop: '4px' }}>
            View your personal lead contact attempts and discussion metrics
          </p>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', flex: '1 1 300px', maxWidth: '400px' }}>
          <input
            type="text"
            placeholder="Search by customer name, vehicle, or comments..."
            value={search}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '850px' }}>
              <thead>
                <tr style={{ borderBottom: '1.5px solid var(--db-bd, rgba(0,0,0,0.08))', background: 'var(--db-sf2, #f9f9f9)' }}>
                  ...
                  <th style={{ padding: '1.125rem 1.5rem', fontSize: '0.75rem', fontWeight: 800, color: 'var(--db-tx2, #555)', textTransform: 'uppercase' }}>Interest / Vehicle</th>
                ...
                {filtered.map(c => {
                  ...
                  return (
                    <tr key={c.id} style={{ borderBottom: '1px solid var(--db-bd, rgba(0,0,0,0.05))', fontSize: '0.875rem' }}>
                      ...
                      <td style={{ padding: '1.25rem 1.5rem' }}>
                        <Link href={`/employee/crm/leads/${c.lead_id}`} style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', color: '#E10613', fontWeight: 700, textDecoration: 'none' }} className="hover-link">
