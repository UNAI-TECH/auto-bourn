...
export default function EmployeeBookingsPage() {
  ...
  const handleClaimRequest = async (id: string) => {
    ...
    try {
      ...
      console.error(err);
      showToast('Failed to claim reservation', 'error');
    } finally {
      setUpdatingId(null);
    }
  };
  ...
  const myActiveCount = requests.filter(r => r.employee_id === employee?.id && r.delivery_status === 'processing').length;
  const myCompletedCount = requests.filter(r => r.employee_id === employee?.id && r.delivery_status === 'completed').length;
    ...
    <div className="db-page">
      ...
      <div className="stats-row">
        ...
        <div className="stat-card" style={{ borderLeft: '4px solid #3b82f6' }}>
          ...
          <div>
            <span className="stat-val">{myActiveCount}</span>
            <span className="stat-lbl">My Booked Cars</span>
          </div>
        </div>
        <div className="stat-card" style={{ borderLeft: '4px solid #22c55e' }}>
          ...
          <div>
            ...
            <span className="stat-lbl">My Delivered Sales</span>
          </div>
        </div>
