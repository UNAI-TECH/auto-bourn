...
export default function EmployeeBookingsPage() {
  ...
  const handleClaimRequest = async (id: string) => {
    ...
    try {
      ...
      showToast('Reservation request claimed successfully!');
      loadRequests();
    } catch (err) {
      console.error(err);
      showToast('Failed to claim reservation');
    } finally {
      setUpdatingId(null);
    }
  };
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'completed':
        return { bg: 'rgba(34, 197, 94, 0.08)', color: '#22c55e', label: 'Delivered (Sold)' };
      case 'cancelled':
        return { bg: 'rgba(239, 68, 68, 0.08)', color: '#ef4444', label: 'Cancelled' };
      case 'processing':
        return { bg: 'rgba(245, 158, 11, 0.08)', color: '#f59e0b', label: 'Booked (Active)' };
      default:
        return { bg: 'rgba(59, 130, 246, 0.08)', color: '#3b82f6', label: 'Pending Request' };
    }
  };
    ...
    <div style={{ padding: 0 }}>
      <div className="db-page-header">
        <div className="db-page-title-container">
          ...
          <p className="db-page-sub">View and manage customer booking/reservation requests from the website</p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        {loading ? (
          ...
          <p style={{ color: 'var(--db-tx2, #555555)', textAlign: 'center', padding: '2rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                    {isUnassigned ? (
                      <button
                        ...
                        className="claim-btn"
                      >
                        <Bookmark size={12} /> Claim Request
                      </button>
                    ) : isAssignedToMe ? (
